...
Volcano_Plot <- function(DEGAnalysis,
  ...
  for (i in seq_along(lfc.col)) {
    ...
    df <- df.orig %>% 
      ...
      dplyr::mutate(
        !!lfccol := tidyr::replace_na(.data[[lfccol]], 0),
        !!sigcol := tidyr::replace_na(.data[[sigcol]], 1)
      )
    ...
    if (isTRUE(use_custom_lab)) {
      if (nchar(change_lfc_name) > 0) {
        lfc_name <- change_lfc_name
      }
      if (nchar(change_sig_name) > 0) {
        sig_name <- change_sig_name
      }
    ...
    # Create static plot
    p <- EnhancedVolcano::EnhancedVolcano(df, x = lfc_name, y = sig_name,
                         lab = df[, label.col],
    ...
    p_empty <- EnhancedVolcano::EnhancedVolcano(df, x = lfc_name, y = sig_name,
                               lab = rep("", nrow(df)),
                               selectLab = NULL,
                               title = title,
                               subtitle = group,
    ...
    interactive_plot <- plotly::ggplotly(pxx, tooltip = c("text"))
