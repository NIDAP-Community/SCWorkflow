...
#' @param show_plots Logical; if TRUE, print and plot trajectory figures as side effects.
#' @param return_results Logical; if TRUE, return a list containing plot objects and summary data.
#'
#' @return If `return_results = TRUE`, returns a list with plot objects and pseudotime summaries.
...
Trajectory_CellTypes <- function(Seurat_Object,
  ...
  dimplot <- Seurat::DimPlot(seurat_object)
  tsne_compare <- cowplot::plot_grid(plot_tsne2, plot_tsne3, ncol = 2)
  umap_compare <- cowplot::plot_grid(plot_umap2, plot_umap3, ncol = 2)

  heatmap_up <- scater::plotHeatmap(
    sce[, on_first_path],
  ...
  heatmap_down <- scater::plotHeatmap(
    sce[, on_first_path],
    order_columns_by = "Pseudotime",
    colour_columns_by = "label",
  ...
  if (length(custom_gene_list) > 0) {
    ...
    if (isTRUE(show_plots)) {
      print(custom_scatter)
    }

    custom_heatmap <- scater::plotHeatmap(
      sce[, on_first_path],
      order_columns_by = "Pseudotime",
      colour_columns_by = "label",
      features = custom_gene_list,
  ...
  result <- list(
    plots = list(
      dimplot = dimplot,
      tsne_mst = plot_tsne1,
      tsne_pseudotime = plot_tsne2,
      tsne_mnn = plot_tsne3,
      tsne_compare = tsne_compare,
      umap_mst = plot_umap1,
      umap_pseudotime = plot_umap2,
      umap_mnn = plot_umap3,
      umap_compare = umap_compare,
      top_decrease = plot_decrease,
      top_increase = plot_increase,
      heatmap_up = heatmap_up,
      heatmap_down = heatmap_down,
      custom_scatter = custom_scatter,
      custom_heatmap = custom_heatmap
    ),
    data = list(
      pseudotime = sce$Pseudotime,
      top_up = top_up,
      top_down = top_down,
      custom_genes = custom_gene_list,
      pseudo_table = sorted_tests
    )
  )
...
ensure_single_layer_assay <- function(seurat_object) {
  active_assay <- Seurat::DefaultAssay(seurat_object)
  joined_object <- tryCatch(
    SeuratObject::JoinLayers(seurat_object, assay = active_assay),
    error = function(e) NULL
  )

  if (!is.null(joined_object)) {
    return(joined_object)
  }

  if ("RNA" %in% Seurat::Assays(seurat_object)) {
    Seurat::DefaultAssay(seurat_object) <- "RNA"
    joined_rna <- tryCatch(
      SeuratObject::JoinLayers(seurat_object, assay = "RNA"),
      error = function(e) NULL
    )
    if (!is.null(joined_rna)) {
      return(joined_rna)
    }
  }
