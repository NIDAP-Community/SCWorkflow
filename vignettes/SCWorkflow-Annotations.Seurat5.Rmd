...
r_file_names <- c("Annotate_Cell_Types.R", "Add_External_Annotation.R", "Dual_Labeling.R",
                  "Color_by_Genes_Automatic.R", "ModuleScore.R", 
                  "Name_Clusters_by_Enriched_Cell_Type.R", "Dotplot_by_Metadata.R")
invisible(sapply(r_file_names, function(f) {
...
Comb_SO=list()
.load_rds_fallback <- function(candidates) {
  for (p in candidates) {
    if (file.exists(p)) {
      return(readRDS(p))
    }
  }
  stop(paste("None of the expected files were found:", paste(candidates, collapse = ", ")))
}

Comb_SO_raw <- .load_rds_fallback(c(
  "./images/CN_SO.Seurat5.rds",
  "./images/CN_SO_Seurat5.rds",
  "./images/CN_SO.rds"
))
Comb_SO$object <- if (inherits(Comb_SO_raw, "Seurat")) Comb_SO_raw else Comb_SO_raw$object
...
Anno_SO=annotateCellTypes(object=Comb_SO$object,
                          ...
                          legend.dot.size = 2,
                          do.finetuning = FALSE,
                          local.celldex = "../backup_seurat5_migration_20260311/CellDexDatabase.rds",
                          use.clusters = NULL
                          )
...
ggsave(Anno_SO$plots$p1, filename = "./images/Anno1.png", width = 10, height = 10)
ggsave(Anno_SO$plots$p2, filename = "./images/Anno2.png", width = 10, height = 10)
...
Anno_SO$object@meta.data[, 'mouseRNAseq_main', drop=F] %>% dplyr::rename(`Cell Type` = mouseRNAseq_main) %>% rownames_to_column('Barcode') %>%
   head()%>%write.table("./images/PerCell_Metadata.csv",sep=",",row.names=F,quote=F)
...
CellType_Anno_Table=read.csv("./images/PerCell_Metadata.csv", check.names = FALSE)
...
ExtAnno_SO=AddExternalAnnotation(object = Anno_SO$object,
...
grep('Cd4', rownames(Anno_SO$object), ignore.case=T, value=T)

dl_assay <- if ("SCT" %in% names(Seurat::Assays(Anno_SO$object))) "SCT" else "RNA"

DLAnno_SO=dualLabeling(object = Anno_SO$object, 
                        samples = c("PBS","CD8dep","ENT","NHSIL12","Combo"), 
                        marker.1="Nos2", 
                        marker.2="Arg1", 
                        marker.1.type = dl_assay, 
                        marker.2.type = dl_assay, 
                        data.reduction = "both", 
                        point.size = 0.5, 
                        point.shape = 16, 
...
plot(DLAnno_SO$plots$densityHM)
dev.off()
...
```{r,eval=run_Chunks,echo=T,results='hide',fig.width=10,fig.asp=.9}
  cbm_assay <- if ("SCT" %in% names(Seurat::Assays(Anno_SO$object))) "SCT" else "RNA"
...
clstrTable=data.table(OriginalClusterIDs= c(0,1,2,3,4,5,6,7),
                      NewClusterNames=c("Pop0", "Pop1", "Pop2", "Pop3","Pop4", "Pop5", "Pop6", "Pop7")
)
write.table(clstrTable,
            file = "./images/Cluster_Names.txt",
            sep = '\t',
            row.names = F)
  clstrTable%>%knitr::kable()
...
ggsave(FigOut$plots, filename = "./images/DPM.png", width = 9, height = 6)
