...
`compareCellPopulations()` returns two complementary visualizations:
- **Barplot**: stacked group-level composition with alluvial-style flows between groups to show changes in relative structure.
- **Boxplot**: per-sample distributions for each annotated cell population, faceted by cluster/cell type.
...
- `annotation.column` should be a stable cell identity label (for example `immgen_main`, `seurat_clusters`, or curated cell types).
- `group.column` should represent the condition being compared (for example `Treatment`).
- `sample.column` should uniquely identify biological samples/replicates (default `orig.ident`).
- `group.order` can be set to enforce display order when plotting conditions.

**Methodology**  
The method first aggregates metadata by annotation and group to compute percentages and counts. It then links these summaries to sample-level metadata and generates (1) a composition-focused barplot and (2) a faceted boxplot for sample-level variability. Together, these plots help distinguish overall compositional shifts from replicate-level dispersion.
...
![](./images/Vis_CCPbar.png)
![](./images/Vis_CCPbox.png)
