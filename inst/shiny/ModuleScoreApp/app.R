...
# reformat user input celltype markers for modScore to recognize
...
server <- function(input, output, session) {
  ...
  gene_choice_rv <- reactiveVal(character()) # all possible markers in SCT@data
  thresholds_rv <- reactiveVal(setNames(numeric(0), character(0)))

  # canonical snapshot of marker list for cache decisions
  canonicalize_markers <- function(x) {
    if (length(x) == 0) return(list())
    nms <- sort(names(x))
    out <- lapply(nms, function(n) sort(unique(x[[n]])))
    names(out) <- nms
    out
  }
  last_markers_snapshot <- reactiveVal(NULL)
  ...
  compute_fun <- if (has_scworkflow) getFromNamespace("compute_modscore_data", "SCWorkflow") else compute_modscore_data
    ...
    # cache previously computed values, don't recalculate again downstream if 
    ## no change occurred with marker table
    ms_data_cached <- reactive({
      args <- compute_args()
  ...
  observeEvent(input$compute, {
    ...
    ms_data_rv(res)
    # remember the marker state we computed for
    last_markers_snapshot(current_snapshot)
  ...
  observe({
    ...
    for (ct in cts) {
      local({
        ...
        thr_re <- throttle(reactive(input[[id]]),100)
